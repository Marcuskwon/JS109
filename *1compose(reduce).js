...
The function will return the value that results from the first parameter being used as a parameter 
for all of the accepted function parameters in turn. So:
*/
...
function compose(number, ...fun) {
  

  return fun.reduce((acc, curr) => {
    return curr(acc);
  }, number);
